GetAdvisorQuoteWorkflowDb() {
    ...
    urls := Map(
        ...
        "ascProductContains", "/ASCPRODUCT/110/"
    )
...
AdvisorNormalizeVehicleDescriptor(rawVehicle) {
    ...
    if (text = "")
        return Map("raw", rawVehicle, "year", "", "make", "", "model", "", "trimHint", "", "displayKey", "")

    year := ""
    if RegExMatch(text, "i)\b((19|20)\d{2})\b", &my)
        year := my[1]

    withoutYear := (year != "") ? Trim(RegExReplace(text, "i)\b" year "\b", "")) : text
