...
- RAPPORT checks `gather_rapport_snapshot` before normal vehicle-loop work and routes an active Gather Edit Vehicle panel through the existing edit handler before continuing. Drivers/Vehicles checks `asc_drivers_vehicles_snapshot` before row reconciliation and routes active ASC Remove Driver or inline participant panels through the existing modal handler before continuing.
