...
- The Drivers/Vehicles ledger handles active ASC Remove Driver modals, inline participant panels, primary driver add, conservative spouse resolution, extra driver removal, ASC vehicle membership rows, and the final Save and Continue gate. Existing action ops still perform mutations; snapshots and ledger reads do not.
- Default ASC business policy remains conservative: explicit lead Single wins, spouse override is disabled by default, Married requires an exact spouse or unique age-window candidate, and ambiguous spouse candidates fail safe. The default remove-driver reason remains code `0006` / "This driver has their own car insurance".
