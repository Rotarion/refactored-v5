...
Confirmed count: 65 unique top-level Advisor JS op names are called by AHK. The smoke guard `testAhkCalledAdvisorOperatorInventoryExistsInRuntime()` in `tests/advisor_quote_ops_smoke.js` asserts these 65 names still exist as `case` labels in generated runtime `assets/js/advisor_quote/ops_result.js`.
...
| `advisor_state_snapshot` | `workflows/advisor_quote_workflow.ahk` / `AdvisorQuoteCaptureStateSnapshotRaw()` | Read-only route/status snapshot for debug hotkey and observer. | Read-only JSON | JSON with `route`, `confidence`, `anchors`, `blockers`, `product`, `selectProduct`, `ascDriversVehicles`, `insuranceGate`, `prefillGate`, `rapport`, `iframe`, `allowedNextActions`, `unsafeReason`. Stable routes include `ENTRY_CREATE_FORM`, `CUSTOMER_SUMMARY_PREFILL_GATE`, `PRODUCT_OVERVIEW`, `RAPPORT`, `SELECT_PRODUCT`, `CONSUMER_REPORTS`, `ASC_DRIVERS_VEHICLES`, `ASC_EXTRA_INFO_INSURANCE`, `ASC_CREDIT_HIT_NOT_RECEIVED`, `ASC_PRIOR_INSURANCE_NOT_FOUND`, `COVERAGES`, `PURCHASE`, `ADVISOR_OTHER`, `UNKNOWN_UNSAFE`. The snapshot remains read-only; recognized ASCPRODUCT insurance/credit gates may expose provisional next-action evidence, while the separate mutating ops enforce exact route/kind/control evidence and client-verification flags. |
...
| `asc_credit_hit_not_received_continue` | `workflows/advisor/advisor_quote_consumer_reports.ahk` / `AdvisorQuoteTryRouteRecognizedAscInsuranceGate()` | Continue past the exact credit-hit-not-received gate without filling fields. | Mutating | Key/value result. Requires `advisor_state_snapshot` route `ASC_CREDIT_HIT_NOT_RECEIVED`, `insuranceGate.kind=CREDIT_HIT_NOT_RECEIVED`, one enabled Continue/Next action, sets `creditHitNotReceived=1`, `source=SYSTEM_DETECTED_GATE`, `requiresClientVerification=1`, and `filledFieldCount=0`. |
...
| `asc_extra_info_insurance_apply_provisional` | `workflows/advisor/advisor_quote_consumer_reports.ahk` / `AdvisorQuoteTryRouteRecognizedAscInsuranceGate()` | Apply agency provisional defaults for the exact extra-info insurance gate. | Mutating | Key/value result. Requires route `ASC_EXTRA_INFO_INSURANCE`, `insuranceGate.kind=EXTRA_INFO_INSURANCE`, scoped controls `priorInsurance_insurerName` and `priorInsurance_lenTimeContinuousPriorInsuranceMonthCnt`, no BI-limit/expiration controls, readback match for `Other` and `3+ years`, one enabled Continue/Next action, `source=PROVISIONAL_AGENCY_DEFAULT`, and `requiresClientVerification=1`. |
...
| `asc_prior_insurance_not_found_apply_provisional` | `workflows/advisor/advisor_quote_consumer_reports.ahk` / `AdvisorQuoteTryRouteRecognizedAscInsuranceGate()` | Apply agency provisional defaults for the exact prior-insurance-not-found gate. | Mutating | Key/value result. Requires route `ASC_PRIOR_INSURANCE_NOT_FOUND`, `insuranceGate.kind=PRIOR_INSURANCE_NOT_FOUND`, scoped controls `priorInsurance_insurerName`, `priorInsurance_lenTimePreviousInsurer`, `priorInsurance_limitAmountText`, `contractTermExpDt`, and start-date evidence, readback match for `Other`, `5+ years`, `I do not know`, and current-year `12-31-YYYY`, one enabled Continue/Next action, `source=PROVISIONAL_AGENCY_DEFAULT`, and `requiresClientVerification=1`. |
