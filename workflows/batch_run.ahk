...
WriteLatestBatchLeadList(raw) {
    global latestBatchLeadListFile

    text := Trim(String(raw ?? ""), "`r`n `t")
    if (text = "")
        return

    try FileDelete(latestBatchLeadListFile)
    FileAppend(text "`n", latestBatchLeadListFile, "UTF-8")
}
